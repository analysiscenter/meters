...
class NearestDetection(ResNet18):
    def build_config(self, names=None):
        ...
        config['head']['units'] = (4 + 1) * num_digits
        return config
    ...
    def head(self, inputs, num_digits, name='head', **kwargs):
        ...
        all_confidences = tf.get_default_graph().get_tensor_by_name("NearestDetection/inputs/confidence:0")
        ...
        for index in range(num_digits):
            ...
            all_dists = []
            # print('current_normalized ', current_normalized[:, 0].get_shape().as_list())
            # print('outputs ', outputs[:, 0].get_shape().as_list())

            for j in range(num_digits):
                all_dists.append(tf.square(current_normalized[:, 1] - outputs[:, 5 * j + 1]))
        ...
        for index in range(num_digits):
            ...
            predicted_bb.append(outputs[:, 5 * min_index: 5 * min_index + 4])
            predicted_confidences.append(outputs[:, 5 * min_index + 4 : 5 * (min_index + 1)])
            all_predictions.append(outputs[:, 5 * min_index: 5 * (min_index + 1)])

        predicted_bb = tf.stack(predicted_bb, axis=1)
        predicted_confidences = tf.stack(predicted_confidences, axis=1)
        all_predictions = tf.stack(all_predictions, axis=1)

        tf.identity(predicted_bb, name='predicted_bb')
        tf.identity(predicted_confidences, name='predicted_confidences')
        # tf.identity(all_predictions, name='all_predictions')
        tf.identity(outputs, name='all_predictions')

        predicted_confidences = tf.reshape(predicted_confidences, [-1, 1])
        all_confidences = tf.reshape(all_confidences, [-1, 1])
        print('all_confidences shape', all_confidences.get_shape().as_list())
        print('predicted_confidences shape', predicted_confidences.get_shape().as_list())

        ce_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(labels=all_confidences, logits=predicted_confidences))
        ...
        mse_loss =  tf.reduce_mean(tf.square(all_coordinates - predicted_bb))
...
class ClassificationModel(ResNet18):
    ...
    def head(self, inputs, name='head', **kwargs):
        kwargs = self.fill_params('head', **kwargs)
        outputs = super().head(inputs, **kwargs)
        # outputs = conv_block(inputs, name=name, **kwargs)
        # print('outputs.shape ', outputs.shape)
        return outputs
